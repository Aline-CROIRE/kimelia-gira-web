...
import { LayoutDashboard, Home, Heart, MessageSquare, User, LogOut } from 'lucide-react';
    ...
    <DashboardWrapper>
      <Sidebar>
        <div style={{ marginBottom: '35px', padding: '0 20px' }}>
           <p style={{ fontSize: '0.6rem', fontWeight: 800, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '2px' }}>Account</p>
           <h5 style={{ color: '#0B397F', fontFamily: 'Space Grotesk', marginTop: '5px' }}>{user?.role?.toUpperCase()}</h5>
        </div>

        <NavItem $active={activeTab === 'overview'} onClick={() => setActiveTab('overview')}>
          <LayoutDashboard size={18}/> <span>Overview</span>
        </NavItem>
        {isPro && <NavItem $active={activeTab === 'properties'} onClick={() => setActiveTab('properties')}><Home size={18}/> <span>Portfolio</span></NavItem>}
        <NavItem $active={activeTab === 'favorites'} onClick={() => setActiveTab('favorites')}><Heart size={18}/> <span>Watchlist</span></NavItem>
        <NavItem $active={activeTab === 'messages'} onClick={() => setActiveTab('messages')}><MessageSquare size={18}/> <span>Messages</span></NavItem>

        <div style={{ marginTop: 'auto', paddingTop: '20px', borderTop: '1px solid #F1F5F9' }}>
            <NavItem onClick={logout} style={{ color: '#EF4444' }}><LogOut size={18}/> <span>Secure Exit</span></NavItem>
        </div>
      </Sidebar>
